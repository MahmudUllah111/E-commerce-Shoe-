...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Ensure sample reviews for all products (2-3 per product) for development/testing
        $customerIds = DB::table('users')->whereIn('email', ['mahmud@gmail.com','customer@trustedmart.com','mahmudshuharto500@gmail.com','nusratankhi115@gmail.com'])->pluck('id')->toArray();
        if (empty($customerIds)) $customerIds = DB::table('users')->where('role','customer')->pluck('id')->toArray();
        if (empty($customerIds)) $customerIds = DB::table('users')->pluck('id')->toArray();
        $reviewComments = [
            'Super comfortable right out of the box. The fit is true to size and the material feels premium.',
            'Great shoes for daily wear. Lightweight and the cushioning is excellent for long walks.',
            'Looks exactly like the photos. Received lots of compliments already. Would buy again!',
            'Solid build quality but runs slightly narrow. Order half size up if you have wide feet.',
            'Good value for money. Shipping was fast and the packaging was neat. Happy with this purchase.',
            'These are my new go-to casual shoes. Breathable, stylish, and very comfortable.',
            'The color is a bit different from the picture but still looks great. Comfortable all day.',
            'Bought these for the gym and they perform well. Good grip and supportive sole.',
            'Very pleased with the quality. The leather feels genuine and the stitching is clean.',
            'Decent shoes for the price. Not the softest but break in nicely after a few wears.',
            'Perfect for my office commute. Professional look with athletic comfort.',
            'Kids love them and they seem durable. Easy to clean and the design is fun.',
        ];
        $reviewNames = ['Rafi Ahmed','Sadia Karim','Tanvir Hossain','Nusrat Jahan','Imran Hasan','Mehrab Ali'];
        $reviewRatings = [5,4,5,3,4,5,4,5,3,4,5,4];

        $allProductIds = DB::table('products')->pluck('id')->toArray();
        foreach ($allProductIds as $pid) {
            $existingCount = DB::table('reviews')->where('product_id', $pid)->count();
            ...
            for ($r=0; $r<$numReviews; $r++) {
                ...
                $commentIdx = array_rand($reviewComments);
                $rating = $reviewRatings[$commentIdx];
                $daysAgo = rand(1,60);
                $createdAt = now()->subDays($daysAgo);
                DB::table('reviews')->insert([
                    'product_id' => $pid,
                    'user_id' => $custId,
                    'user_name' => $reviewNames[$commentIdx % count($reviewNames)],
                    'author_name' => $reviewNames[$commentIdx % count($reviewNames)],
                    'rating' => $rating,
            ...
            if (!empty($approvedRatings)) {
                $avgRating = round(array_sum($approvedRatings) / count($approvedRatings), 1);
                $reviewCount = count($approvedRatings);
        ...
        foreach ([
            [
                'slug'=>'about',
                'title'=>'About Us',
                'content'=>'<h1>About TrustedMart</h1><p>TrustedMart is a modern footwear retail platform built for everyone — from daily commuters to performance athletes. We curate sneakers, formals, sports shoes, and casual wear from brands you love.</p><p>Our mission is simple: quality shoes, fair prices, fast delivery, and genuine customer support. Every pair is sourced from authorized distributors.</p><p>We are based in Dhaka and ship across Bangladesh with multiple payment options including Cash on Delivery.</p>'
            ],
            [
                'slug'=>'faq',
                'title'=>'FAQ',
                'content'=>'<h1>Frequently Asked Questions</h1>
                <h2>Ordering</h2><p>Browse products, choose size/color, add to cart, and checkout. You can order as guest or signed-in customer.</p>
                <h2>Shoe sizing</h2><p>We show US sizes on product pages. If you are between sizes, we recommend going up half a size for sneakers and sticking to your usual size for formals.</p>
                <h2>Order cancellation/change</h2><p>Contact us within 2 hours of placing an order for cancellation or changes. After dispatch, changes may not be possible.</p>
                <h2>Delivery time</h2><p>Inside Dhaka: 2–3 business days. Outside Dhaka: 3–5 business days. Express shipping is available at checkout.</p>
                <h2>Tracking</h2><p>Use the order number from your confirmation email on the <a href="/track-order">Track Order</a> page.</p>
                <h2>Unavailable Size/Color</h2><p>Add the product to your Wishlist and submit a restock request with your preferred size and color. We will email you when it is back.</p>
                <h2>Wishlist/restock requests</h2><p>Wishlist lets you save favorites. If an item is out of stock, submit a restock request and we will notify you automatically when inventory is replenished.</p>
                <h2>Payments</h2><p>We support Cash on Delivery, bKash, Nagad, and SSLCommerz (Visa/Mastercard). No payment is charged on the website unless you complete the gateway flow.</p>
                <h2>Returns</h2><p>30-day return policy for unworn shoes in original packaging. Contact support to initiate a return.</p>
                <h2>Support</h2><p>Email us at <a href="mailto:support@trustedmart.com">support@trustedmart.com</a> or use the <a href="/contact">Contact Us</a> page.</p>'
            ],
            [
                'slug'=>'return-policy',
                'title'=>'Return Policy',
                'content'=>'<h1>Return Policy</h1><p>We want you to love your shoes. If you are not satisfied, you may return unworn items within 30 days of delivery for a full refund or exchange.</p><h2>Conditions</h2><ul><li>Shoes must be unworn, unwashed, and in original packaging.</li><li>Tags and accessories must be intact.</li><li>Custom or discounted final-sale items are non-returnable.</li></ul><h2>How to return</h2><p>Contact our support team with your order number. We will provide a return address and instructions.</p><p>Refunds are processed within 5–7 business days after we receive and inspect the returned item.</p>'
            ],
            [
                'slug'=>'terms',
                'title'=>'Terms & Conditions',
                'content'=>'<h1>Terms & Conditions</h1><p>By using TrustedMart, you agree to the following terms:</p><ul><li>All prices are in USD and subject to change without notice.</li><li>We reserve the right to cancel orders suspected of fraud or abuse.</li><li>Product colors may vary slightly from screen display.</li><li>We are not liable for delays caused by courier partners or force majeure events.</li><li>Your personal data is handled per our Privacy Policy.</li></ul><p>For questions, contact <a href="mailto:support@trustedmart.com">support@trustedmart.com</a>.</p>'
            ],
            [
                'slug'=>'privacy',
                'title'=>'Privacy Policy',
                'content'=>'<h1>Privacy Policy</h1><p>TrustedMart respects your privacy. We collect only the information necessary to process orders, improve service, and communicate updates.</p><h2>What we collect</h2><ul><li>Name, email, phone, and shipping address for order fulfillment.</li><li>Payment info is handled by secure third-party gateways; we do not store card details.</li><li>Cookies and analytics data to improve site performance.</li></ul><h2>How we use it</h2><ul><li>Process and deliver orders.</li><li>Send order confirmations and shipping updates.</li><li>Respond to customer support inquiries.</li></ul><h2>Your rights</h2><p>You may request data correction or deletion by contacting support. We do not sell your personal data to third parties.</p>'
            ],
        ] as $p) {
