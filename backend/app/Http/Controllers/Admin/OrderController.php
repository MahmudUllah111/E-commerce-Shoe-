...
class OrderController extends Controller
{
    ...
    public function updateStatus(Request $request, Order $order)
    {
        ...
        // store internal note if provided (use contact_messages or simple log; we will create a order_notes table alternative via DB json? For now store in DB via a generic table if exists else ignore)
        // We'll attempt to store in a file or just flash; spec says internal notes – we will save to a new table order_notes if exists, else we create a simple json column? For now use cache/session flash and optional DB insert if table exists.
        if (!empty($data['internal_note'])) {
            try {
                if (\Illuminate\Support\Facades\Schema::hasTable('order_notes')) {
                    ...
                    // fallback: append to shipping_address? No, just log
        ...
        if ($request->boolean('send_email') && !empty($order->customer_email)) {
            try {
                $order->load(['items.product', 'items.productVariant', 'user']);
                $note = "Status changed from {$old} to " . strtoupper($data['status']) . "." . (!empty($data['internal_note']) ? " Note: {$data['internal_note']}" : "");
                Mail::to($order->customer_email)->send(new OrderConfirmationMail(
                    $order,
                    "Order #{$order->order_number} Update: " . ucfirst($data['status']),
                    $note
                ));
            } catch (\Throwable $e) {
