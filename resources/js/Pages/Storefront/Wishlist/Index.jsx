...
import { Trash2, Bell, X, Check, ShoppingBag, ArrowRight } from 'lucide-react';
...
export default function Index({ items = [] }){
    ...
    const submitRequest = (product, e) => {
        ...
        router.post(route('wishlist.request'), {
            ...
            onError: (errs) => {
                setSubmitting(prev => ({ ...prev, [product.id]: false }));
                toast.error(Object.values(errs)[0] || 'Failed to save request');
            },
        });
    };
    ...
    return (
        <StorefrontLayout>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
                <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2 border-b border-gray-200 pb-5">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 font-semibold mt-1">
                            {user ? 'Your saved favorites • Set restock alerts or purchase directly' : 'Sign in to save and manage your favorites'}
                        </p>
                    </div>
                ...
                {!user && (
                    <div className="mt-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 text-sm font-semibold text-amber-800 flex items-center justify-between flex-wrap gap-3">
                        <span>Please sign in to keep your wishlist permanently synced across all devices.</span>
                        <Link href={route('login')} className="px-4 py-1.5 bg-slate-900 text-white rounded-full text-xs font-bold hover:bg-black">Sign In</Link>
                    </div>
                )}

                {items.length === 0 ? (
                    <div className="mt-8 bg-white border border-gray-200 rounded-2xl p-12 text-center max-w-lg mx-auto shadow-sm">
                        <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4 text-gray-400">
                            <ShoppingBag size={28} />
                        </div>
                        <h3 className="text-lg font-bold text-slate-900">Your wishlist is empty</h3>
                        <p className="text-sm text-gray-500 mt-1 mb-6">Explore our latest footwear releases and save your favorites.</p>
