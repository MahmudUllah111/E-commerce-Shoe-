...
export default function Index({ reviews, filters={} }){
    ...
    const editForm = useForm({ rating: 5, comment: '' });
    ...
    const startEdit = (r) => {
        setEditId(r.id);
        editForm.setData({ rating: r.rating, comment: r.comment });
    };
    const submitEdit = (r) => {
        editForm.patch(route('admin.reviews.update', r.id), {
            preserveScroll: true,
            onSuccess: () => { toast.success('Review updated'); setEditId(null); },
                                        ...
                                        {editId === r.id ? (
                                            <div className="space-y-2">
                                                <div className="flex gap-1">
                                                    {[1,2,3,4,5].map(n=>(
                                                        <button key={n} type="button" onClick={()=>editForm.setData('rating', n)} className={`w-8 h-8 rounded-full border font-black text-xs ${editForm.data.rating===n?'bg-slate-900 text-white border-slate-900':'bg-white'}`}>{n}</button>
                                                    ))}
                                                </div>
                                                <textarea value={editForm.data.comment} onChange={e=>editForm.setData('comment', e.target.value)} rows={3} className="w-full px-2 py-1.5 border rounded text-xs" />
                                                <div className="flex gap-1">
                                                    <button onClick={()=>submitEdit(r)} disabled={editForm.processing} className="px-2 py-1 bg-slate-900 text-white rounded text-xs font-bold">Save</button>
                                                    <button onClick={()=>setEditId(null)} className="px-2 py-1 bg-white border rounded text-xs font-bold">Cancel</button>
                                                </div>
                                            </div>
                                        ) : (
                                            <>
                                                <p className="line-clamp-2 text-gray-700 font-medium">{r.comment}</p>
                                                {r.admin_reply && <div className="mt-2 p-2 bg-slate-50 border rounded text-xs"><span className="font-black">Reply:</span> {r.admin_reply}</div>}
                                            </>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        <div className="flex flex-col gap-1 items-end">
                                            <div className="flex gap-1">
                                                ...
                                                {editId !== r.id ? (
                                                    <button onClick={()=>startEdit(r)} className="px-2 py-1 bg-white border border-gray-200 rounded text-xs font-bold">Edit</button>
                                                ) : null}
