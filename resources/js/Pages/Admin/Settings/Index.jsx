...
export default function Index({ settings, mailSettings = {}, mailLogs = [], users, roles }){
    ...
    const mailForm = useForm({
        ...
        mail_host: mailSettings.mail_host || 'smtp.gmail.com',
        ...
        mail_from_address: mailSettings.mail_from_address || 'mahmudsets@gmail.com',
        mail_from_name: mailSettings.mail_from_name || 'TrustedMart',
    });
    ...
    const submitMail = (e) => {
        e.preventDefault();
        mailForm.post(route('admin.settings.mail.update'), {
            preserveScroll: true,
            onSuccess: () => toast.success('SMTP email settings saved!'),
            onError: (errs) => toast.error(Object.values(errs)[0] || 'Failed to save SMTP settings'),
        });
    };

    const handleSendTestMail = (e) => {
        e.preventDefault();
        if (!testEmail) {
        ...
        setTestingMail(true);
        router.post(route('admin.settings.mail.test'), { test_email: testEmail }, {
            preserveScroll: true,
            onFinish: () => setTestingMail(false),
            onSuccess: () => toast.success(`Test email sent to ${testEmail}!`),
            onError: (errs) => toast.error(Object.values(errs)[0] || 'SMTP test email failed'),
        });
    };
        ...
        <AdminLayout header="Store Settings">
            ...
            {tab==='mail' && (
                <div className="space-y-6">
                    <form onSubmit={submitMail} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className="font-black text-slate-900 text-lg">Outgoing SMTP Server Settings</h3>
                                <p className="text-xs text-gray-500 font-semibold mt-1">
                                    Configure credentials to send automated order confirmations, shipping updates, restock notifications, and inquiry replies.
                                </p>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider ${mailForm.data.mail_mailer === 'smtp' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                                Mode: {mailForm.data.mail_mailer.toUpperCase()}
                            </span>
                        </div>

                        {/* Quick Presets */}
                        <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                            <span className="text-xs font-black uppercase text-slate-600 block mb-2">⚡ Quick Provider Presets:</span>
                            <div className="flex flex-wrap gap-2">
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        toast.success('Applied Gmail SMTP settings!');
                                    }}
                                    className="px-3 py-1 bg-white border border-gray-300 rounded-lg text-xs font-bold hover:bg-gray-50 shadow-sm"
                                >
                                    Google / Gmail
                                </button>
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        toast.success('Applied Mailtrap settings!');
                                    }}
                                    className="px-3 py-1 bg-white border border-gray-300 rounded-lg text-xs font-bold hover:bg-gray-50 shadow-sm"
                                >
                                    Mailtrap (Sandbox)
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        toast.success('Applied Brevo settings!');
                                    }}
                                    className="px-3 py-1 bg-white border border-gray-300 rounded-lg text-xs font-bold hover:bg-gray-50 shadow-sm"
                                >
                                    Brevo (Sendinblue)
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        toast.success('Switched to local Log driver');
                                    }}
                                    className="px-3 py-1 bg-white border border-gray-300 rounded-lg text-xs font-bold hover:bg-gray-50 shadow-sm"
                                >
                                    Local Log Mode
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">Mail Driver / Mailer *</label>
                                <select
                                    value={mailForm.data.mail_mailer}
                                    onChange={e=>mailForm.setData('mail_mailer', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-bold bg-white"
                                >
                                    <option value="smtp">SMTP (Real Outgoing Server — Gmail, Mailtrap, Brevo, SendGrid, etc.)</option>
                                    <option value="log">Log (Testing only: writes email contents to storage/logs/laravel.log)</option>
                                    <option value="sendmail">Sendmail (Local server sendmail binary)</option>
                                </select>
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">SMTP Host *</label>
                                <input
                                    type="text"
                                    value={mailForm.data.mail_host}
                                    onChange={e=>mailForm.setData('mail_host', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-mono font-bold"
                                    placeholder="smtp.gmail.com or sandbox.smtp.mailtrap.io"
                                />
                                {mailForm.errors.mail_host && <div className="text-xs text-rose-600 mt-1">{mailForm.errors.mail_host}</div>}
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">SMTP Port *</label>
                                <input
                                    type="number"
                                ...
                                />
                                {mailForm.errors.mail_port && <div className="text-xs text-rose-600 mt-1">{mailForm.errors.mail_port}</div>}
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">Encryption Scheme</label>
                                <select
                                    value={mailForm.data.mail_encryption}
                                    onChange={e=>mailForm.setData('mail_encryption', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-bold bg-white"
                                >
                                    <option value="tls">TLS / STARTTLS (Default for port 587 / 2525)</option>
                                    <option value="ssl">SSL / SMTPS (Required for port 465)</option>
                                    <option value="none">None (Plain unencrypted)</option>
                                </select>
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">SMTP Username</label>
                                <input
                                    type="text"
                                    value={mailForm.data.mail_username}
                                    onChange={e=>mailForm.setData('mail_username', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-medium"
                                    placeholder="e.g. user@gmail.com or API key"
                                />
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">SMTP Password / App Password</label>
                                <input
                                    type="password"
                                    value={mailForm.data.mail_password}
                                    onChange={e=>mailForm.setData('mail_password', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-mono"
                                    placeholder="••••••••••••••••"
                                />
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">Default Sender Email ("From" Address) *</label>
                                <input
                                    type="email"
                                    value={mailForm.data.mail_from_address}
                                    onChange={e=>mailForm.setData('mail_from_address', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-semibold"
                                    placeholder="support@trustedmart.com"
                                />
                                {mailForm.errors.mail_from_address && <div className="text-xs text-rose-600 mt-1">{mailForm.errors.mail_from_address}</div>}
                            </div>

                            <div>
                                <label className="text-xs font-black uppercase text-gray-500">Default Sender Name ("From" Name) *</label>
                                <input
                                    type="text"
                                    value={mailForm.data.mail_from_name}
                                    onChange={e=>mailForm.setData('mail_from_name', e.target.value)}
                                    className="mt-1 w-full px-3 py-2.5 border rounded-lg text-sm font-bold"
                                    placeholder="TrustedMart Support"
                                />
                                {mailForm.errors.mail_from_name && <div className="text-xs text-rose-600 mt-1">{mailForm.errors.mail_from_name}</div>}
                            </div>
                        </div>
                        ...
                        <div className="pt-2">
                            <button
                                ...
                                className="px-6 py-2.5 bg-slate-900 text-white rounded-full font-black text-sm hover:bg-black disabled:opacity-60 transition"
                            >
                                {mailForm.processing ? 'Saving...' : 'Save SMTP Settings'}
                            </button>
                        </div>
                    </form>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-6">
                        <h4 className="font-black text-slate-900">Verify SMTP Configuration (Send Test Email)</h4>
                        <p className="text-xs text-slate-500 font-medium mt-1 mb-4">
                            Send a real test email through your configured SMTP server to confirm host, port, authentication, and delivery.
                        </p>
                        <form onSubmit={handleSendTestMail} className="flex flex-col sm:flex-row gap-3 max-w-xl">
                            <input
                                ...
                                className="flex-1 px-4 py-2.5 border border-slate-300 rounded-lg text-sm font-medium bg-white outline-none focus:ring-1 focus:ring-slate-900"
                            />
                            <button
                                type="submit"
                                disabled={testingMail || !testEmail}
                                className="px-5 py-2.5 bg-rose-600 text-white font-black text-sm rounded-lg hover:bg-rose-700 disabled:opacity-60 transition flex items-center justify-center gap-2 whitespace-nowrap"
                            >
                                {testingMail ? 'Connecting & Sending...' : '✉️ Send Test Email'}
                            </button>
                        </form>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-6">
                        <div className="flex justify-between items-center mb-4">
                            <div>
                                <h4 className="font-black text-slate-900">Recent Email Activity Log ({mailLogs.length})</h4>
                                <p className="text-xs text-gray-500 font-medium mt-0.5">Real-time log of customer inquiries, alerts, order emails, and direct replies.</p>
                            </div>
                            <span className="text-[11px] font-bold text-gray-400">Auto-saved to MySQL</span>
                        </div>

                        {mailLogs.length === 0 ? (
                            <div className="text-center py-8 text-xs font-semibold text-gray-400">
                                No email logs recorded yet. Try sending a test email above!
                            </div>
                        ) : (
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {mailLogs.map(log => (
                                            <tr key={log.id} className="hover:bg-gray-50/70">
                                                ...
                                                <td className="px-3.5 py-2.5 font-bold capitalize text-slate-700">
                                                    {log.type.replace('_', ' ')}
                                                </td>
                                                <td className="px-3.5 py-2.5 font-medium text-slate-900">
                                                    {log.to_email}
                                                </td>
                                                <td className="px-3.5 py-2.5 text-gray-700 font-semibold max-w-xs truncate" title={log.subject}>
                                                    {log.subject}
                                                </td>
                                                <td className="px-3.5 py-2.5">
                                                    <span className={`px-2 py-0.5 rounded-full font-black text-[10px] uppercase tracking-wider ${log.status === 'sent' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
                                                        {log.status}
                                                    </span>
                                                </td>
                                            </tr>
